...
export function calculateExercises(): Result {
    ...
    const objetivoUsuario = document.getElementById('T_TARGET') as HTMLInputElement;
    ...
    for (let i = 1; i < DIAS_SEMANA; i++) {
        ...
        const indiceDias = document.getElementById(idDia) as HTMLInputElement | null;
...
function giveRateBasedOnRating(hours: number, target: number): 1 | 2 | 3 {
    if (hours >= target) return 3;
    else if (hours >= target * 0.7) return 2;
    else return 1;
}

function descriptionBasedOnRating(rate: number): string {
    if (rate == 1)
